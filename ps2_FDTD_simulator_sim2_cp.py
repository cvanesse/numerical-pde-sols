# Wave diffraction FDTD simulation code for ECE 570 problem set #2

## IMPORT MODULES
from finite_difference_methods_cp import *
from matplotlib import pyplot as plt
import time

sparse = cp._cupyx.scipy.sparse
...
x_barrier = np.array([9.5e-6, 10e-6])
slot_width = 5e-6
...
A = 1 # Pulse amplitude
source_x = cp.array([1e-6])
source_y = cp.array([0.5e-6, 29e-6])
...
print("Timestep: \t\t\t {:0.2e} seconds".format(dt.item()))
...
print("Simulation Time: \t {:0.2e} seconds".format(T.item()))

print("------------------------")
print("-- Initializing Solutions and Operators...")
# Allocate space for solution vectors
u = [cp.zeros(domain["shape"].tolist()),
     cp.zeros(domain["shape"].tolist()),
     cp.zeros(domain["shape"].tolist())] # Solution is a list of arrays

# Construct refractive index matrix
n = n_bg*np.ones(domain["shape"].tolist()) # Start in cartesian coordinates

idx_barrier = np.around(x_barrier / domain['h'][1].item()).astype('int64')
y_barrier = 15e-6 + np.array([-slot_width/2, slot_width/2])
idy_barrier = np.around(y_barrier/domain['h'][0].item()).astype('int64')
...
n = np.reshape(n, [domain["size"].item()], order="F")
n = np.power(n, -2)
n = sparse.diags(n, format="csr")

n = cp._cupyx.scipy.sparse.csr_matrix(n)
...
source_idy = cp.around(source_y / domain['h'][0].item()).astype('int64')
source_idy = cp.array([cp.min(source_idy).item(), cp.max(source_idy).item()])
...
for i in range(Nt):
    ...
    u[2][cp.min(source_idy).item():cp.max(source_idy).item(), source_idx] = A*math.sin(omega*t)
